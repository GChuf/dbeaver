...
import org.jkiss.dbeaver.ui.app.standalone.rpc.BoberKurwaInstanceServer;
 ...
 */
public class BoberKurwaApplication extends DesktopApplicationImpl implements DBPApplicationDesktop, DBPApplicationController {

    private static final Log log = Log.getLog(BoberKurwaApplication.class);
    ...
    static BoberKurwaApplication instance;
    ...
    private BoberKurwaInstanceServer instanceServer;
    ...
    public BoberKurwaApplication() {
        this(DesktopPlatform.DBEAVER_DATA_DIR, DEFAULT_WORKSPACE_FOLDER);
    }

    protected BoberKurwaApplication(String defaultWorkspaceLocation, String defaultAppWorkspaceName) {
        ...
        // Windows: %AppData%/BoberKurwaData
        // MacOS: ~/Library/BoberKurwaData
        // Linux: $XDG_DATA_HOME/BoberKurwaData
    ...
    /**
     * Gets singleton instance of BoberKurwa application
     * @return application or null if application wasn't started or was stopped.
     */
    public static BoberKurwaApplication getInstance() {
        return instance;
    }
    ...
    public Object start(IApplicationContext context) {
        ...
        CommandLine commandLine = BoberKurwaCommandLine.getCommandLine();
        String defaultHomePath = getDefaultInstanceLocation();
        if (BoberKurwaCommandLine.handleCommandLine(commandLine, defaultHomePath)) {
            if (!Log.isQuietMode()) {
        ...
        try {
            headlessMode = true;
            if (BoberKurwaCommandLine.handleCustomParameters(commandLine)) {
                return IApplication.EXIT_OK;
            }
        } finally {
            headlessMode = false;
        ...
        // Run instance server
        try {
            instanceServer = BoberKurwaInstanceServer.createServer();
        } catch (Exception e) {
...
*/
        ...
        try {
            ...
            if (shell != null) {
                ...
                Log.Listener splashListener = (message, t) -> {
                    BoberKurwaSplashHandler.showMessage(CommonUtils.toString(message));
                };
                Log.addListener(splashListener);
                shell.addDisposeListener(e -> {
                    Log.removeListener(splashListener);
                });
                BoberKurwaSplashHandler.showMessage("Starting " + Platform.getProduct().getName());
            }
        } catch (Throwable e) {
                ...
                if (instanceLoc.isLocked() || !instanceLoc.set(defaultHomeURL, true)) {
                    if (exclusiveMode || reuseWorkspace) {
                        ...
                        int msgResult = showMessageBox(
                            "BoberKurwa - Can't lock workspace",
                            "Can't lock workspace at " + defaultHomePath + ".\n" +
                                "It seems that you have another BoberKurwa instance running.\n" +
    ...
    protected boolean tryMigrateFromPreviousVersion(Path homeDir) {
        ...
        if (previousVersionWorkspaceDir != null) {
            BoberKurwaSettingsImporter importer = new BoberKurwaSettingsImporter(this, getDisplay());
    ...
    private void shutdown() {
        log.debug("BoberKurwa is stopping"); //$NON-NLS-1$

        saveStartupActions();

        try {
            BoberKurwaInstanceServer server = instanceServer;
            if (server != null) {
                instanceServer = null;
            ...
            log.debug("BoberKurwa shutdown completed"); //$NON-NLS-1$
    ...
    private void initDebugWriter() {
        DBPPreferenceStore preferenceStore = BoberKurwaActivator.getInstance().getPreferences();
        if (!preferenceStore.getBoolean(BoberKurwaPreferences.LOGS_DEBUG_ENABLED)) {
            return;
        }
        String logLocation = preferenceStore.getString(BoberKurwaPreferences.LOGS_DEBUG_LOCATION);
    ...
    public IInstanceController createInstanceClient() {
        return BoberKurwaInstanceServer.createClient(getDefaultInstanceLocation());
    }
    ...
    private void saveStartupActions() {
        ...
        if (!props.isEmpty()) {
            ...
            try (Writer writer = Files.newBufferedWriter(path)) {
                props.store(writer, "BoberKurwa startup actions");
            } catch (Exception e) {
