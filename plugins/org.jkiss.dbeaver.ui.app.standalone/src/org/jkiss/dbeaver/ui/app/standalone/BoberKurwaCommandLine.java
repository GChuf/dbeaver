 ...
 * 1. On BoberKurwa start. It tries to find already running BoberKurwa instance (thru REST API) and make it execute passed commands
 *    If BoberKurwa will execute at least one command using remote invocation then application won't start.
 *    Otherwise it will start normally (and then will try to process commands in UI)
 * 2. After BoberKurwa UI start. It will execute commands directly
 */
public class BoberKurwaCommandLine
{
    private static final Log log = Log.getLog(BoberKurwaCommandLine.class);
    ...
    public final static Options ALL_OPTIONS = new Options()
        ...
        .addOption(PARAM_STOP, "quit", false, "Stop BoberKurwa running instance")
        ...
        .addOption(PARAM_BRING_TO_FRONT, PARAM_BRING_TO_FRONT, false, "Bring BoberKurwa window on top of other applications")
    ...
    static boolean executeCommandLineCommands(@Nullable CommandLine commandLine, @Nullable IInstanceController controller, boolean uiActivated) throws Exception {
        ...
        if (commandLine.hasOption(PARAM_REUSE_WORKSPACE)) {
            if (BoberKurwaApplication.instance != null) {
                BoberKurwaApplication.instance.setReuseWorkspace(true);
            }
        }
    ...
    static boolean handleCommandLine(CommandLine commandLine, String instanceLoc) {
        ...
        if (commandLine.hasOption(PARAM_HELP)) {
            ...
            helpFormatter.printHelp("dbeaver", GeneralUtils.getProductTitle(), ALL_OPTIONS, "(C) 2010-2024 BoberKurwa Corp", true);
            return true;
        }
        ...
        for (ParameterDescriptor param : customParameters.values()) {
            if (param.exclusiveMode && (commandLine.hasOption(param.name) || commandLine.hasOption(param.longName))) {
                if (BoberKurwaApplication.instance != null) {
                    BoberKurwaApplication.instance.setExclusiveMode(true);
                }
        ...
        try {
            IInstanceController client = BoberKurwaInstanceServer.createClient(instanceLoc);
