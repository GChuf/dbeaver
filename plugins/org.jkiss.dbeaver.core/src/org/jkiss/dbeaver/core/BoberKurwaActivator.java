 ...
 */
public class BoberKurwaActivator extends AbstractUIPlugin {

    // The shared instance
    private static BoberKurwaActivator instance;
    private ResourceBundle pluginResourceBundle, coreResourceBundle;
    private PrintStream debugWriter;
    private DBPPreferenceStore preferences;

    public BoberKurwaActivator() {
    }

    public static BoberKurwaActivator getInstance() {
        return instance;
    }
    ...
    public void start(BundleContext context)
        ...
        if (getPreferenceStore().getBoolean(BoberKurwaPreferences.UI_USE_EMBEDDED_AUTH)) {
            try {
                if (Desktop.isDesktopSupported()) {
                    injectProxyPeer();
                } else {
                    getLog().warn("Desktop interface not available");
                    getPreferenceStore().setValue(BoberKurwaPreferences.UI_USE_EMBEDDED_AUTH, false);
                }
            } catch (Throwable e) {
